...
public class Article {
    ...
    ///
    /// To understand one-to-many and bidirectional, consider
    /// 2 paradigms: Object oriented (OO) and Relational database.
    ///
    ...
    /// Since Article is dependent on `List<Comment>` and Comment is dependent on Article, so this is a recursive-chain call which we have to break,
    ...
    /// Did you notice we are using `Set<Tag>` and not `List<Tag>`? → Reason is hibernate mapping creates an issue if we try to remove one associated tag to this article.
    /// For e.g. if we remove one tag, hibernate would delete all the tags and re-insert N-1 tags which causes performance degradation.
    @JsonIgnore
    @ToString.Exclude
